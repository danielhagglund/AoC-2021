...
def check_bingo(boards):
    ...
    return {'winners': winners, 'remaining': remaining }
...
winning_board = []
last_winning_drawing_number = 0

for num in drawnumbers:
    mark_boards(boards, int(num))
    winners = []
    bingo_results = check_bingo(boards)
    last_winning_drawing_number = int(num)
    if (len(bingo_results['winners'])) > 0:
        winning_board = bingo_results['winners'].pop()
    if len(bingo_results['remaining']) > 0:
        boards = bingo_results['remaining']
    else:
        break
        
final_score = sum_all_unpicked_numbers(winning_board) * last_winning_drawing_number
